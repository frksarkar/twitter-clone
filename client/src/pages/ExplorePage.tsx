...
const ExplorePage = () => {
	...
	const mediaContent = mockTweets.filter((tweet) => tweet.media && tweet.media.length > 0);
			...
			<section>
				...
				<div className="space-y-1">
					{mockTrends.map((trend) => (
						<div
							key={trend._id}
							className="px-4 py-3 hover:bg-secondary-50 dark:hover:bg-secondary-800 transition-colors cursor-pointer"
						>
				...
				<div className="grid grid-cols-2 gap-0.5 p-1">
					{mediaContent.map((tweet) =>
						tweet.media?.map((image, index) => (
							<div key={`${tweet._id}-${index}`} className="aspect-square overflow-hidden">
								<img
									src={image}
									alt="Media content"
